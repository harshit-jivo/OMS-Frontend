  ...
  * most specific patterns first. `enrich` adds runtime data (e.g. parsed amounts). */
  ...
  /**
   * The two failures a fresh look at the warehouse can actually fix.
   *
   * Named, and exported, because Invoice Review offers a different button for
   * them — "Re-check batches & repost" instead of a plain Retry, which would
   * only send the same stale batch numbers back. A second copy of these
   * patterns there would eventually disagree with the copy that writes the
   * message the reviewer is reading.
   *
   * `negative inventory` is SAP B1's own wording when a line would take a
   * warehouse below zero, and it says neither "stock" nor "insufficient" — so
   * without it here the block that most often needs a re-allocation was the one
   * that did not match.
   */
  export const BATCH_ERROR = /batch|serial/i;
  export const STOCK_ERROR =
    /insufficient|out\s*of\s*stock|not\s*enough|available\s*quantity|stock|on[-\s]?hand|negative\s*inventory|falls?\s*into\s*negative/i;

  /** Would re-reading the warehouse and allocating again plausibly fix this? */
  export const isBatchOrStockError = (message: unknown): boolean => {
    const text = String(message ?? "");
    return BATCH_ERROR.test(text) || STOCK_ERROR.test(text);
  };
  ...
  const RULES: Rule[] = [
    ...
    // Batch / serial allocation problems.
    {
      ...
      result: {
        ...
        action: "The batches on this invoice may have been sold or moved since it was built. Re-check batches to allocate from what is in the warehouse now.",
        transient: false,
      },
    },
    // Insufficient / unavailable stock, including SAP's negative-inventory block.
    {
      match: STOCK_ERROR,
      result: {
        title: "Not Enough Stock",
        summary: "There isn't enough available stock to fulfil one or more items.",
        action: "Re-check batches to allocate from current stock, or review the quantities.",
        transient: false,
      },
    },
